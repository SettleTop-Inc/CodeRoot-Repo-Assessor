...
# predicate `build_source` already uses.
...
def test_build_cache_never_touches_the_mcp_client_seam_when_unconfigured(monkeypatch):
    """Same shape as the build_source guard above: a configured-vs-not test
...
def test_source_and_cache_never_disagree_about_mcp_vs_direct(monkeypatch):
    ...
    that used a different predicate than build_source -- not just one that
    ...
    for kwargs in ({}, {"coderoot_mcp_url": "http://mcp.local"}):
        ...
        source_is_mcp = isinstance(wiring.build_source(s), McpSource)
